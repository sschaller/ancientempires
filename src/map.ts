...
class Tilemap {
    ...
    load() {
        ...
        for (let x = 0; x < this.width; x++) {
            ...
            for (let y = 0; y < this.height; y++) {
                ...
                let tile = Tilemap.getTileForCode(code);
                this.tiles[x][y] = tile;
    ...
    getCostAt(position: Pos, entity_type: EntityType) {
        return Tilemap.getCostForTile(this.getTileAt(position), entity_type);
    }
    getDefAt(position: Pos, entity_type: EntityType) {
        return Tilemap.getDefForTile(this.getTileAt(position), entity_type);
    }
    isCampaign(): boolean {
